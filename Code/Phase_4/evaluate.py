...
def evaluate_RNN(param,
    ...
    count = np.zeros((no_of_class, no_of_class), dtype=np.int32)
    predictions = []
    gen_data = get_minibatch(data, param['batch_size'] )
    gen_sent = get_minibatch(sentences, param['batch_size'])
    for batch, val in enumerate(zip(gen_data,gen_sent)):
        ...
        char_ids, \
        word_lengths, \
        labels = prepare_input_rnn(data)
        feed_dict = {
            rnn.word_ids: word_ids,
            rnn.sequence_lengths: sequence_lengths,
            rnn.char_ids: char_ids,
            rnn.word_lengths: word_lengths,
            rnn.dropout: np.asarray(1),
        }
        labels_pred = []
        if param['crf'] != 0 :
            logits, trans_params = sess.run([rnn.logits, rnn.trans_params], feed_dict=feed_dict)
            ...
            labels_pred = sess.run(rnn.labels_pred, feed_dict=feed_dict)

        for i, sent_label in enumerate(labels_pred):

            y_preds = labels_pred[i][0:sequence_lengths[i]]
            y_reals = labels[i][0:sequence_lengths[i]]

            p_tags = [id_to_tag[y_pred] for y_pred in y_preds]
            r_tags = [id_to_tag[y_real] for y_real in y_reals]

            if param['label_scheme'] == 2:
                p_tags = iobes_iob(p_tags)
                r_tags = iobes_iob(r_tags)
            for j, (y_pred, y_real) in enumerate(zip(y_preds, y_reals)):
                new_line = " ".join(sentences[i][j][:-1] + [r_tags[j], p_tags[j]])
                predictions.append(new_line)
                count[y_real, y_pred] += 1
            predictions.append("")
